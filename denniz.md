![Installation Guide For Project Elixir](https://i.imgur.com/3UmK6nS.png "Installation")

### Installation guide for Project Elixir for OnePlus Nord2 5G (denniz)

### Clean Flash (RECOMMENDED):
1. Go to Recovery
2. Make sure you are on A.20 Firmware
3. Flash Rom
4. Format Data and cache
5. Reboot

### for magisk
6. Reboot to recovery after first boot
7. Flash Magisk zip/apk
8. Reboot to system

### Dirty flash (For updates):
1. Go to recovery
2. Wipe cache, dalvik cache
3. Flash Rom
4. Reboot

### Note
If coming from Another ROM then use format data(type 'yes') or perform fastboot -w
ROM based on A.20 firmware, so make sure u are on A.20 firmware.
